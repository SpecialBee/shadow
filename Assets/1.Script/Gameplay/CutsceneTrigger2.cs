...
namespace ShadowSeller.Core
{
    public class CutsceneTrigger2 : MonoBehaviour
    {
        ...
        [Tooltip("이 이름의 아이템이 인벤토리에 있어야 트리거 발동. 비워두면 조건 없음.")]
        ...
        [Header("엔딩 연출")]
        [Tooltip("체크하면 컷씬 종료 후 엔딩 로고 실행")]
        [SerializeField] private bool playEnding = false;
        ...
        private void Fire()
        {
            ...
            if (!string.IsNullOrEmpty(requiredItemName) &&
                (InventoryManager.Instance == null || !InventoryManager.Instance.HasItem(requiredItemName)))
                return;
            ...
            if (director == null)
            {
                Debug.LogWarning("[CutsceneTrigger2] CutsceneDirector가 씬에 없습니다.");
                return;
            }
